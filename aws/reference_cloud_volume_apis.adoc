...
This section shows you how to use the Cloud Volumes APIs.  The examples use curl from a Linux bash shell. You need to replace `<api_url>`, `<api_key>`, and `<secret_key>` with the values you recorded from link:finding_urL_key_secretKey[Finding the API URL, API key, and Secret key].
